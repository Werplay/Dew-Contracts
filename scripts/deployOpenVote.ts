...
import { OpenVoting } from "../typechain-types";
...
async function main() {
	...
	const openVoting = (await upgrades.deployProxy(
		OpenVoting,
		[
	...
	console.log(" openVoting Proxy deployed at : ", openVoting.address);
	...
	if (openVoting.address == addresses.proxy) {
		const args = [
			configService.getValue("CONTRACT_OWNER"),
			configService.getValue("TOKEN_ADDRESS"),
		];
		...
		await verifyProxy(openVoting.address);
	}
}
